...
export default class MidataService {
  ...
  public createObservation(
    ...
    return new Promise((resolve, reject) => {
      ...
      if (observationType == ObservationType.BODY_TEMPERATURE) {
        observation = this.newBtObservation(
          _status,
          bodySite,
          values[0],
          dateString
        ...
        observation = this.newHrObservation(
          _status,
          bodySite,
          values[0],
          dateString
        ...
        observation = this.newBpObservation(
          _status,
          bodySite,
          values[0],
          values[1],
  ...
  updateObservation(
    ...
    return new Promise((resolve, reject) => {
      this.jsOnFhir
        .getResource('Observation', _id)
        .then((result) => {
          if (result) {
            const fhirObservation = result as Observation;
            fhirObservation.bodySite = this.getBodySiteFromJson(
              bodySite,
              observationType
            );
            fhirObservation.method = this.getMethodFromJson(
              bodySite,
              observationType
            );
  ...
  newBtObservation(
    ...
    return {
      ...
      bodySite: this.getBodySiteFromJson(
        bodySite,
        ObservationType.BODY_TEMPERATURE
      ),
      method: this.getMethodFromJson(
        bodySite,
        ObservationType.BODY_TEMPERATURE
      ),
    };
  ...
  getBodySiteFromJson(bodySite: string, observationType: ObservationType) {
    ...
    throw new Error(
      `No matching body Site found in fhirJson for input [${bodySite}]`
    );
  }
  ...
  async generateRandomData(dateString: string) {
    const bodyTemperaturesBase = [
      36, 36.5, 37, 37.5, 38, 38, 38.5, 39.5, 41, 39, 38.5, 40, 41, 39.5, 38,
      36.5,
    ];
    ...
    const heartRateBase = [
      70, 70, 75, 80, 90, 95, 105, 110, 115, 120, 110, 100, 95, 85, 80, 70,
    ];
    ...
    const sysPressure = [
      145, 142, 149, 151, 153, 147, 149, 153, 150, 154, 162, 168, 163, 156, 150,
      151,
    ];
    ...
    const diasPressure = [
      87, 84, 90, 91, 92, 90, 94, 92, 96, 104, 98, 93, 92, 86, 90, 87,
    ];
    ...
    for (let i = 0; i < heartRateBase.length; i++) {
      ...
      const offset = Math.round(
        Math.random() * (60 * 60 * 12 * 1000 - 60 * 60 * 4 * 1000) +
          60 * 60 * 4 * 1000
      );
      dat = dat - offset;
    ...
    try {
      ...
      Notify.create({
        message:
          '48 randomisierte Observationen wurden erstellt. Bitte laden Sie die Seite neu',
        color: 'green',
        position: 'top',
        icon: 'announcement',
      });
  ...
  getRandomBodySite(observationType: ObservationType): string {
    if (observationType == ObservationType.BODY_TEMPERATURE) {
      return fhirDataJson.BODY_TEMPERATURE[
        Math.floor(Math.random() * fhirDataJson.BODY_TEMPERATURE.length)
      ].bodySite.coding[0].display;
    } else if (observationType == ObservationType.HEART_RATE) {
      return fhirDataJson.HEART_RATE[
        Math.floor(Math.random() * fhirDataJson.HEART_RATE.length)
      ].bodySite.coding[0].display;
    } else if (observationType == ObservationType.BLOOD_PRESSURE) {
      return fhirDataJson.BLOOD_PRESSURE[
        Math.floor(Math.random() * fhirDataJson.BLOOD_PRESSURE.length)
      ].bodySite.coding[0].display;
    } else {
      throw new Error('No matching Observation Type found');
    }
